...
export default function TheoryPage() {
  ...
  // Filter lessons based on selected language tab
  const filteredLessons = theoryLessons.filter((lesson) => lesson.category === activeTab);

  // Overall progress
  const completedOverall = game.completedTheoryLessons.length;
  const totalOverall = theoryLessons.length;
  const overallPercentage = Math.round((completedOverall / totalOverall) * 100) || 0;

  // JS Progress
  const completedJs = theoryLessons
    .filter((l) => l.category === 'JS')
    .filter((l) => game.completedTheoryLessons.includes(l.id)).length;
  const totalJs = theoryLessons.filter((l) => l.category === 'JS').length;

  // TS Progress
  const completedTs = theoryLessons
    .filter((l) => l.category === 'TS')
    .filter((l) => game.completedTheoryLessons.includes(l.id)).length;
  const totalTs = theoryLessons.filter((l) => l.category === 'TS').length;
  ...
  // Simple markdown renderer helper for custom theoretical guides (no backticks)
  ...
  return (
    <div className="flex-1 bg-background text-foreground py-12 px-6 md:px-12 flex flex-col items-center font-sans">
      ...
      <div className="w-full max-w-3xl bg-card border-[3px] border-border rounded-2xl p-6 mb-8 shadow-[4px_4px_0px_var(--shadow-color)]">
        <div className="flex justify-between items-center text-[10px] font-game font-extrabold text-slate-550 dark:text-slate-400 uppercase tracking-wide mb-3">
          ...
          <span>
            {completedOverall}/{totalOverall} ({overallPercentage}%)
          </span>
        </div>
        <Progress value={overallPercentage} color="purple" className="h-5" />
      </div>

      {/* Track Selection Tabs (Brutalist / Pop Art tabs) */}
      <div className="flex gap-4 mb-8 w-full max-w-3xl justify-center">
        ...
        >
          ...
          JavaScript
          <span className="text-[10px] opacity-70 ml-1">({completedJs}/{totalJs})</span>
        </button>
        <button
          onClick={() => {
            playClickSound();
        ...
        >
          ...
          TypeScript
          <span className="text-[10px] opacity-75 ml-1">({completedTs}/{totalTs})</span>
        </button>
      </div>

      {/* Filtered Lessons list */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl flex-1 items-stretch">
          ...
          return (
            ...
            >
              <Card
                ...
                className="h-full flex flex-col justify-between hover:border-retro-orange transition-colors w-full"
              >
                <div>
            ...
            >
              ...
              <div className="p-6 border-b-[3px] border-dashed border-border-muted bg-slate-950/10">
                <span className="text-[9px] font-game font-extrabold text-slate-500 uppercase tracking-wider block mb-1">
                  {selectedLesson.category === 'JS' ? 'JavaScript' : 'TypeScript'}
                </span>
